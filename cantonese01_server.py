# pip install python-dotenv requests flask flask-cors waitress llama-cpp-python edge-tts
# Skills: chat, speak-cantonese, speak-cantonese-save
import os
import sys
import re
import time
...
API_PORT     = 5074
STATIC_PORT  = 8114
INDEX_FILE   = "cantonese01_index.html"

# ── Import skills from skills/ subfolder ─────────────────────────────────────

sys.path.insert(0, os.path.join(SKILLS_DIR, "speak-cantonese", "scripts"))
from speak_cantonese import speak_nonblocking as speak_cantonese

sys.path.insert(0, os.path.join(SKILLS_DIR, "speak-cantonese-save", "scripts"))
from speak_cantonese_save import speak_and_save as speak_cantonese_save

sys.path.insert(0, os.path.join(SKILLS_DIR, "speak-cantonese-file", "scripts"))
from speak_cantonese_file import speak_cantonese_file
...
# ── Skill: speak-cantonese ────────────────────────────────────────────────────

@app.route("/api/speak-cantonese", methods=["POST"])
def api_speak_cantonese():
    data     = request.get_json()
...
# ── Skill: speak-cantonese-save ───────────────────────────────────────────────

@app.route("/api/speak-cantonese-save", methods=["POST"])
def api_speak_cantonese_save():
    data     = request.get_json()
    message  = (data.get("message") or "").strip()
...
def api_speak_cantonese_file():
    ...
    if not filename:
        return jsonify({"answer": "Please include a filename.\nExample: use skill speak-cantonese-file cantonese-challenge-1.txt"})
    ...
    lines = [l for l in lines if len(l) >= 1 and _re.search(r'\w', l)]
